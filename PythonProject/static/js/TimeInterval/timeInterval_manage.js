...
function initTimeIntervalManageTool() {
	timeInterval_manage_tool = {
		...
		search: function() {
			$("#timeInterval_manage").datagrid("options").queryParams=queryParams; 
			$("#timeInterval_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#timeIntervalQueryForm").form({
			    url:"/TimeInterval/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/TimeInterval/update/" + rows[0].intervalId,
					type : "get",
					data : {
