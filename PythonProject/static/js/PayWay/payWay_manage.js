...
function initPayWayManageTool() {
	payWay_manage_tool = {
		...
		search: function() {
			$("#payWay_manage").datagrid("options").queryParams=queryParams; 
			$("#payWay_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#payWayQueryForm").form({
			    url:"/PayWay/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/PayWay/update/" + rows[0].payWayId,
					type : "get",
					data : {
